...
from src.utils.keyboards import get_main_menu_keyboard, get_goto_main_menu
...
async def handle_show_profile(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        logger.error(f"Unexpected error handling callback 'action_show_profile' for user {user_id}: {e}", exc_info=True)
        try:
